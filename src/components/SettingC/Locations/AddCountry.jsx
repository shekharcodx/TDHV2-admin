 ...
 * Allows users to select a country and displays country code
 */
const AddCountry = () => {
              ...
              {selectedCountry && (
                <div className="mb-3">
                  <div className={styles.selectedInfo}>
                    <strong>Selected:</strong> {countriesData.find(c => c.code === selectedCountry)?.name} 
                    <span className={styles.countryCode}>({selectedCountry})</span>
                  </div>
                </div>
              )}
    
              <button type="submit" className={`btn ${styles.submitBtn}`} disabled={!selectedCountry}>
                Submit Country
              </button>
            </form>
          </div>
